...
export default function Checkout() {
  ...
  const handleStartTrial = async () => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Erro ao iniciar teste"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const annualPrice = (plan.priceAnnual / 100).toFixed(2);
  const monthlySavings = (
    ((plan.priceMonthly * 12 - plan.priceAnnual) / (plan.priceMonthly * 12)) *
    100
  ).toFixed(0);
            ...
            <CardContent className="space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-900">
                  <strong>5 dias grátis:</strong> Teste todos os recursos sem
                  custo. Você será cobrado após o período de teste se não
                  cancelar.
                </p>
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-200">
                <span className="text-slate-600">Período de Teste</span>
                <span className="font-semibold text-yellow-600">Grátis por 5 dias</span>
              </div>
            </CardContent>
          </Card>
